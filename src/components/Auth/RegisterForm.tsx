  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        ...
        setError(t('auth.register.error.failed') || 'Registration failed. Please try again.');
      }
    } catch {
      setError(t('auth.register.error.failed') || 'Registration failed. Please try again.');
    } finally {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('auth.register.title')}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-gradient-to-r from-green-600 to-blue-600 hover:from-green-700 hover:to-blue-700 disabled:opacity-60 text-white py-3 rounded-lg flex items-center justify-center space-x-2 font-medium transition-all duration-200"
        >
          {isLoading ? (
