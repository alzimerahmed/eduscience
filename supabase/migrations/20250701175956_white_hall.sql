...
WHERE NOT EXISTS (SELECT 1 FROM subscription_plans WHERE name = 'Student Monthly' AND role = 'student'::user_role AND period = 'monthly')
UNION ALL
SELECT 'Student Annual', 'student'::user_role, 'annual', 5000.00, 'Annual access for students (save 16.7%)'
WHERE NOT EXISTS (SELECT 1 FROM subscription_plans WHERE name = 'Student Annual' AND role = 'student'::user_role AND period = 'annual')
UNION ALL
SELECT 'Teacher Monthly', 'teacher'::user_role, 'monthly', 5000.00, 'Monthly access for teachers'
WHERE NOT EXISTS (SELECT 1 FROM subscription_plans WHERE name = 'Teacher Monthly' AND role = 'teacher'::user_role AND period = 'monthly')
UNION ALL
SELECT 'Teacher Annual', 'teacher'::user_role, 'annual', 50000.00, 'Annual access for teachers (save 16.7%)'
WHERE NOT EXISTS (SELECT 1 FROM subscription_plans WHERE name = 'Teacher Annual' AND role = 'teacher'::user_role AND period = 'annual');
